...
public class RippleEffect : ContentControl
{
    ...
    private void OnReleaseHandler(Ripple r)
    {
        ...
        Task<bool?> RemoveRippleTask(Task _)
        {
            return Dispatcher.UIThread.InvokeAsync(() => PART_RippleCanvasRoot?.Children.Remove(r));
        }

        Task.Delay(Ripple.Duration).ContinueWith(RemoveRippleTask, TaskContinuationOptions.None);
    }
    ...
    private Ripple CreateRipple(PointerPressedEventArgs e, bool center)
    {
        ...
        var r = new Ripple(w, h) { Fill = RippleFill };
